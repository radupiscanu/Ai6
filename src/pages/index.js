...
import { getBardPayload } from 'src/utils/getBardPayload';
...
export default function Index() {
  ...
  const getIntroAndItineraries = async (msg) => {
    ...
    const { bardContext, bardExample, bardMessage } = getBardPayload({
      isFirstMessage: _messageList.length < 2,
      userMessage: msg,
    });

    _messageList.push(bardMessage);
    ...
    let res = null;
    try {
      res = await llmCaller.sendPrompt(bardContext, bardExample, _messageList, 0.1);
    } catch (error) {
