...
export default function Index() {
  ...
  const getIntroAndItineraries = async (msg, isHotelOrRestaurant) => {
    const id = uuidv4();
    ...
    const bard1Examples = isHotelOrRestaurant
      ? {
          examples: [
            {
              input: {
    ...
    const bard1Msg = isHotelOrRestaurant
      ? {
          author: '0',
          content:
  ...
  const handleOnSendMsg = async (msg) => {
    ...
    const hasTypedHotelOrRestaurant = msg.toLowerCase().includes('hotel') || msg.toLowerCase().includes('restaurant');

    const res = await getIntroAndItineraries(msg, hasTypedHotelOrRestaurant);
